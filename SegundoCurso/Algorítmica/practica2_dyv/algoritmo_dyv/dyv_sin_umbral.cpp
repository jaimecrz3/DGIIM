...
struct Punto{
    double x;
    double y;
    int pos_alcanzables; // Número de posiciones alcanzables

    Punto(double i, double j){ // Constructor
        x=i;
        y=j;
        pos_alcanzables=0;
    }
...
bool punto_alcanza(Punto p1, Punto p2){  // p1 alcanza a p2
    ...
    bool alcanza = false;
    if((p1.x > p2.x) && (p1.y > p2.y)) alcanza = true; // Condición para que p1 alcance a p2
...
int main(int argc, char* argv[]) {
    ...
    sort(puntos.begin(),puntos.end(), ordenar_x); // Se ordenan los vectores por abscisas de menor a mayor
    vector <Punto> sol;
    ...
    sort(sol.begin(),sol.end(), ordenar_x); // LOs volvemos a ordenar para que al mostrar los resultados se vea más claro
    for(int i=0; i<sol.size();i++){
        cout << "Punto (" << sol[i].x <<  "," << sol[i].y << "):"  << sol[i].pos_alcanzables << endl;  // Se muestran los resultados
    }
