...
(define (domain tierra-media-ej3c) ; Cabecera que indica el nombre del dominio
  ...
  ; Parametros: 3 Hobbits, Mago y localizacion en la que se forma la Comunidad.
  ; La seleccion concreta de personajes y punto de reunion no se fuerza en el
  ; codigo: queda en manos del planificador
  ; Se usa desigualdad para evitar que el mismo Hobbit ocupe dos parametros
  (:action formarComunidad
    :parameters (?h1 ?h2 ?h3 ?m - Personaje ?l - Localizacion)
    :precondition
      (and
  ...
  ; En este ejercicio la Comunidad reducida esta formada por 3 Hobbits y un Mago
  (:action viajarComunidad
    :parameters (?h1 ?h2 ?h3 ?m - Personaje ?origen ?destino - Localizacion)
    :precondition
      (and
        (comunidadFormada)
        (not (= ?h1 ?h2))
