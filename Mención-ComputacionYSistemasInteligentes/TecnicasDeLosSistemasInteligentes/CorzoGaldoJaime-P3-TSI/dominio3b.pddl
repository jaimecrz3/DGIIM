...
(define (domain tierra-media-ej3b) ; Cabecera que indica el nombre del dominio
  ...
  ; Parametros: 2 Hobbits, Mago y localizacion en la que se forma la Comunidad.
  ...
  (:action formarComunidad
    :parameters (?h1 ?h2 ?m - Personaje ?l - Localizacion)
    :precondition
      (and
  ...
  ; En este ejercicio la Comunidad reducida esta formada por 2 Hobbits y un Mago
  (:action viajarComunidad
    :parameters (?h1 ?h2 ?m - Personaje ?origen ?destino - Localizacion)
    :precondition
      (and
        (comunidadFormada)
        (not (= ?h1 ?h2))
