...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         org_polares = glm::vec3{org_polares.x + da_, org_polares.y + db_, org_polares.z + 0.0};
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         glm::vec3 nuevas_org_cartesianas = Cartesianas(org_polares);
         glm::vec3 desplazamiento = nuevas_org_cartesianas - org_cartesianas;
         ...
         org_cartesianas = nuevas_org_cartesianas;
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         punto_atencion = punto_atencion + (da_*eje->x) + (db_*eje->y);
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         // nota: los ejes no cambian
         float dz_ = dz / 10.0;
         punto_atencion = punto_atencion + (dz_*eje->z);
