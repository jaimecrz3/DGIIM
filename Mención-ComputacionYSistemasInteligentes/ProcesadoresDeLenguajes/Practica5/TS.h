...
typedef enum { OL_Y=0, OL_O=1, OL_IGUAL=3, OL_DISTINTO=4 } atr_operlog; 
...
typedef struct {
    tipoEntrada entrada;
    char *nombre;   // Contiene los caracteres que forman el identificador. En el caso de que TipoEntrada sea marca, no se usa
    dtipo tipoDato;
    unsigned int parametros;   // nº parámetros si es proc
    unsigned int dimensiones;  // para arrays
    int TamDimen1;
    int TamDimen2;
} entradaTS;
...
extern int dimTmp;               /* para controlar las dimensiones en el caso de los arrays */
extern int tam1Tmp;
extern int tam2Tmp;
...
extern int posArray2D;              /* posición en TS de la variable a la que estamos accediendo */
extern int subprogLlamada;
extern dtipo aux_id;
extern entradaTS TS[MAX_TS];

// para los switch
extern char *NombreVarControlActual;      
extern char *EtiquetaSalidaCaseActual;


typedef struct {
    int   atrib;   // atributo en el caso de que se necesite (para operadores, por ejemplo)
    char *lexema;  // para identificadores
    dtipo tipo;  
    ...
    char *codigo;   // Concatenando el codigo generado
    char *nombre;
...
// funciones de la TS, explicaciones en TS.c
...
// generación de codigo, implementadas en GenCod.c
