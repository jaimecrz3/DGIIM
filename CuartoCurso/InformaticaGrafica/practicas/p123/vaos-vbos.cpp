...
void DescrVBOAtribs::crearVBO() 
{
   ...
   //creo el buffer
   glGenBuffers(1, &buffer); //returns n buffer object names in buffers

   // 2. fija este buffer como buffer 'activo' actualmente en el 'target' GL_ARRAY_BUFFER(para vertex attributes)
   //lo pongo como activo, lo "ato" a GL_ARRAY_BUFFER, esta constante me permite diferenciar entre el VBO de atributos y el de indices
   ...
   //creates a new data store for the buffer object currently bound to target. Any pre-existing data store is deleted. 
   //The new data store is created with the specified size in bytes and usage.
   //trasfiero datos de la memoria a la GPU
   //usage is a hint to the GL implementation as to how a buffer object's data store will be accessed. DRAW: The data store contents are modified by the application, and used as the source for GL drawing and image specification commands.
   ...
   // 5. desactivar el buffer despues de usarlo y asorciarlo al VAO
...
void DescrVBOInds::crearVBO( )
{
   ...
   // 3. transferir los datos desde la memoria de la aplicación al VBO en GPU
   //GL_STATIC_DRAW --> para indicar que indices no se va a modificar desde la GPU
...
void DescrVAO::crearVAO()
{
   ...
   // Recorrido del array de punteros
    for (DescrVBOAtribs* ptr : dvbo_atributo) {
         if(ptr != nullptr){
            ptr->crearVBO();
         }
    }
   ...
   for (DescrVBOAtribs* ptr : dvbo_atributo) {
         if(ptr != nullptr){
            if(!atrib_habilitado[i]){
               glDisableVertexAttribArray(ptr->buffer);
            }
         }
         i++;
    }
