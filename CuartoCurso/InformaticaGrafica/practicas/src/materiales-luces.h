...
class TexturaVerticalHorizontal : public Textura
{
   public: TexturaVerticalHorizontal( const std::string & nom, bool vertical );
} ;
...
class Material
{
   ...
   float
      // coeficiente de reflexión ambiente, valor entre 0 y 1 que determina la fraccion de luz reflejada de forma indirecta
      k_amb    = 0.2f,     
      // coeficiente de reflexión difusa, valor entre 0 y 1 que determina la fraccion de luz reflejada en los objetos mate o difusos
      k_dif    = 0.8f, 
      // coeficiente de reflexión pseudo-especular, valor entre 0 y 1 que determina la fraccion de luz reflejada en los objetos brillantes
      //en los cuales dichas zonas brillantes dependen de la posición del observador
      k_pse    = 0.0f,   
      //exponente de brillo para reflexion pseudo-especular, un valor real positivo que permite variar
      //el tamaño de las zonas brillantes (a mayor valor, menor tamaño y más pulida o especular).  
      exp_pse  = 0.0f ;  
} ;
...
class FuenteLuz
{
   ...
   //Las variables de instancia incluyen las coordenadas esféricas (ángulos de longitud y latitud) 
   //del vector de dirección que apunta a la fuente de luz.
   float
