...
void NodoGrafoEscena::visualizarGL(  )
{
   ...
   //Practica 4 -> 1. al inicio, hacer 'push' de la pila de materiales
   ...
   for( unsigned i = 0 ; i < entradas.size() ; i++ ){
      ...
      switch( entradas.at(i).tipo ){ 
         ...
         break ;
         //Practica 4 -> 2. si una entrada es de tipo material, activarlo usando a pila de materiales
         case TipoEntNGE::material :
            aplicacionIG->pila_materiales->activar(entradas.at(i).material);
         break ;
      }
...
void NodoGrafoEscena::visualizarNormalesGL(  )
{
   ...
   for(unsigned i=0; i<entradas.size(); ++i){
      ...
      switch( entradas.at(i).tipo ){ 
         case TipoEntNGE::objeto :
            // si es entrada objeto:
            entradas[i].objeto->visualizarNormalesGL(); // visualizar objeto
         break ;
         case TipoEntNGE::transformacion :
            // si entrada transf.:
            cauce->compMM( *(entradas[i].matriz)); // componer matriz
...
void NodoGrafoEscena::visualizarModoSeleccionGL()
{
   ...
   for(unsigned i=0; i<entradas.size(); i++){
      if(entradas.at(i).tipo == TipoEntNGE::objeto){
         entradas.at(i).objeto->visualizarModoSeleccionGL();
      }else if(entradas.at(i).tipo == TipoEntNGE::transformacion){
         cauce->compMM(*entradas.at(i).matriz);
      }
   }
...
void NodoGrafoEscena::calcularCentroOC()
{
   ...
   if(!centro_calculado){
      ...
      mat4 matriz_modelado(1.0f);
      vec3 centro_total = vec3{0.0, 0.0, 0.0};
      int suma_centros=0;

      for(unsigned i=0; i<entradas.size(); i++){
         //se recorren recursivamente todas las entradas y se calcula el centro de todas las que sean de tipo sub-objeto 
         //(llamando recursivamente al método y transformado el punto obtenido por la correspondiente matriz de modelado de esa entrada).
         if(entradas.at(i).tipo == TipoEntNGE::transformacion){
            matriz_modelado = matriz_modelado*(*entradas.at(i).matriz);
         }else if(entradas.at(i).tipo == TipoEntNGE::objeto){
            entradas.at(i).objeto->calcularCentroOC();
            centro_total = centro_total + vec3(matriz_modelado * vec4(entradas.at(i).objeto->leerCentroOC(), 1.0f));
            suma_centros++;
         }

      }

      //fijar como centro del nodo promedio de dichos centros.
      centro_total.x /= suma_centros;
      centro_total.y /= suma_centros;
      centro_total.z /= suma_centros;

      ponerCentroOC(centro_total);
...
)
{
   ...
   for(unsigned i=0; i<entradas.size(); i++){
         ...
         if(entradas.at(i).tipo == TipoEntNGE::objeto){
            if(entradas.at(i).objeto->buscarObjeto(ident_busc, matriz_modelado, objeto, centro_wc)){
               return true;
            }
         }else if(entradas.at(i).tipo == TipoEntNGE::transformacion){
            matriz_modelado = matriz_modelado * (*entradas.at(i).matriz);
...
NodoCubo24::NodoCubo24(){
   ...
   agregar(new Material(textura_cubo, 0.5, 0.4, 0.7, 100.0));
   agregar(new Cubo24());
...
NodoDiscoP4::NodoDiscoP4()
{
   ...
   Textura* texturaDisco = new Textura("cuadricula.jpg");
   agregar(new Material(texturaDisco, 0.5, 0.8, 0.7, 100));
   agregar( new MallaDiscoP4() );
}

// *********************************************************************
//Clase BeethovenMadera

BeethovenMadera::BeethovenMadera(bool vetas_vertical)
{
   ponerNombre("BeethovenMadera");
   Textura* texturaBeethoven = new TexturaVerticalHorizontal("text-madera.jpg", vetas_vertical);
   agregar(new Material(texturaBeethoven, 0.2, 0.3, 0.2, 35.0));
...
MiEsferaE1::MiEsferaE1(unsigned i, unsigned j)
{
   fila = i;
   n_esfera_fila = j;
   agregar(new Material( 0.7, 0.3, 0.8, 100.0));
   agregar( new Esfera(100,100) );
}

bool MiEsferaE1::cuandoClick(const glm::vec3 & centro_wc){

   std::cout << "Se ha seleccionado la esfera número " << n_esfera_fila+1 << " de la fila número " << fila+1 << std::endl ;
...
GrafoEsferasP5_2::GrafoEsferasP5_2()
{
   ...
   agregar( fila_esferas );
   agregar( glm::rotate( float((2*M_PI)/n_filas_esferas), glm::vec3{ 0.0, 1.0, 0.0 }));
   }
