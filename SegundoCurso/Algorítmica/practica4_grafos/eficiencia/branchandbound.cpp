...
struct nodo{
    ...
    bool operator < (const nodo n) const{
        return (conveniencia > n.conveniencia);
    }
};
...
public:
        ...
        for(int i = 0; i <num_invitados; i++){
            for(int j = 0; j < num_invitados; j++){
                if(i != j){ //Los de la diagonal no nos interesan
                    ...
                    nodo nuevo={i,j, datos.at(i).at(j)};
    ...
    int Estimador1() const{
        ...
        int estimador=0;
           ...
           conveniencia_ = nodos.front().conveniencia;
    ...
    int Estimador3() const{
        ...
        for(int i = 0; i < datos.size(); i++){
            ...
            if(estan_sentados.at(i)==false) {
                int indice_max;
                int otro_max = 0;
                bool usar_otro_max=false;
                for(int j = 0; j < datos.at(i).size(); j++){

                    if(datos.at(i).at(j) > max && usar_otro_max){
                        otro_max=max;
                    }
