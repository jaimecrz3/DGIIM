...
/*
 * Archivo: Lexico.lex
 * Descripción: Especificación léxica para la herramienta Flex.
 * * Este archivo define las reglas y expresiones regulares para el análisis léxico
 * de un lenguaje de programación estructurado similar a C, con soporte para 
 * palabras reservadas en español y operadores extendidos.
 */
%option yylineno
%option noyywrap
%{
    #include "tabla.h"
    #include <string.h>
    typedef struct { int atrib; char* cadena; } YYSTYPE;
    YYSTYPE yylval;
%}
...
int main ( int argc , char ** argv )
{
    ...
    {
        if (yylval.atrib != -1) {
            printf ("[%-*.*s] ::  %s  :: atrib=%d (%s)  \n", COL_LEX, COL_LEX, yytext, yylval.cadena, yylval.atrib, yytext);
        }
        else if (yylval.cadena == "ID") {
        	printf ("[%-*.*s] ::  %s  :: atrib=%s \n", COL_LEX, COL_LEX, yytext, yylval.cadena, yytext);
        }
        else printf ("[%-*.*s] ::  %s \n", COL_LEX, COL_LEX, yytext, yylval.cadena);
