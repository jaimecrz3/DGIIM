...
// -----------------------------------------------------------------------------
// Afecta al parametro activo del objeto
...
void Objeto3D::actualizarEstado( const float dtSec )
{
   ...
   //De esta manera afectamos a todos los parametros y conseguimos que
   //se realizen todas las animaciones a la vez
   /*for( unsigned i = 0 ; i < leerNumParametros() ; i++  )
   {
      if((int)i==ind_par_act){
         tiempo_par_sec[i] += dtSec ;
         actualizarEstadoParametro( i, tiempo_par_sec[i] );
      }
   }*/

   //De esta manera actualizamos solo el parametro activo
   tiempo_par_sec[ind_par_act] += dtSec ;
   actualizarEstadoParametro( ind_par_act, tiempo_par_sec[ind_par_act] );
}
