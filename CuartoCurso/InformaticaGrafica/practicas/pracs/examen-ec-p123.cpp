...
MallaEjercicio3::MallaEjercicio3(unsigned w, unsigned h){
    ...
    prisma_inf->agregar(scale(vec3{float(ancho)/2.0,float(altura)/2.0,float(w)/2.0}));
    prisma_inf->agregar(translate(vec3{1.0,1.0,0.0}));
    ...
    unsigned ind_tras_prisma_sup = prisma_sup->agregar(translate(vec3{0.0, 0.0, 0.0}));

    prisma_sup->agregar(translate(vec3{0.0,float(altura),0.0}));
    prisma_sup->agregar(scale(vec3{float(ancho)/2.0,float(altura)/2.0,float(ancho)/2.0}));
    prisma_sup->agregar(translate(vec3{1.0,1.0,0.0}));
    ...
    pm_tras_sup = prisma_sup->leerPtrMatriz(ind_tras_prisma_sup);
        ...
        case 0:
            {
                float vmin = M_PI/16; 
                float vmax = M_PI/2; 
                float a_oscilante = (vmax + vmin)/2;
                float b_oscilante = (vmax - vmin)/2;
                float n = 0.5f; // Frecuencia de oscilación

                float factor = a_oscilante + b_oscilante*sin(2*M_PI*n*t_sec);
                *pm_tras_inf = scale(vec3{1.0, factor, 1.0});

                float height_inferior = altura*factor;
                float height_sup = height_inferior - altura;
                *pm_tras_sup = translate(vec3{0.0, height_sup, 0.0});
            }
            break;
        ...
        case 1:
            {
                ...
                *pm_rot_sup = translate(vec3{0.0,float(altura),0.0}) * rotate(float(a_oscilante+b_oscilante*sin(M_PI*n*t_sec)), 
                                       vec3{0.0, 0.0, 1.0}) * translate(vec3{0.0,-float(altura),0.0});
